					...
					<?php
						foreach ( $rows as $row ) { ?>
							<tr>
								...
								<td><a href="<?php echo esc_url(add_query_arg(array( 'page' => 'wpdevart-forms', 'task' => 'edit', 'id' => esc_attr($row->id) ), admin_url('admin.php'))); ?>"><?php echo esc_html($row->title); ?></a></td>
								<?php if($role == "administrator"){
                                     $user = $row->user_id;
									 $user_info = get_userdata( $user ); ?>
									<td><a href="<?php echo esc_url(get_edit_user_link( $user )) ?>"><?php echo ($user_info)? esc_html($user_info->user_login) : ""; ?></a></td>
								<?php } ?>	
								<td><a href="<?php echo esc_url(add_query_arg(array( 'page' => 'wpdevart-forms', 'task' => 'duplicate', 'id' => esc_attr($row->id), '_wpdevart_bc_nonce' => wp_create_nonce( 'duplicate_item' ) ), admin_url('admin.php'))); ?>"><?php _e('Duplicate','booking-calendar'); ?></a></td>
								<td><a href="<?php echo esc_url(add_query_arg(array( 'page' => 'wpdevart-forms', 'task' => 'edit', 'id' => esc_attr($row->id) ), admin_url('admin.php'))); ?>"><?php _e('Edit','booking-calendar'); ?></a></td>
								<td><a href="" onclick="wpdevart_set_value('task','delete'); wpdevart_set_value('cur_id','<?php echo esc_attr($row->id); ?>'); wpdevart_form_submit(event, 'forms_form')" ><?php _e('Delete','booking-calendar'); ?></a></td>
							<tr>
					<?php	}
					?>
				</table>
